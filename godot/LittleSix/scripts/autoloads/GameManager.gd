...
func _ready() -> void:
	...
	# Network message handling (Spec 005)
...
func _on_network_message_received(type: String, payload: Dictionary) -> void:
	if type == "JOIN_ACK":
		hide_loading()
		transition_to(GameState.LOBBY)
	elif type == "RACE_COUNTDOWN":
		# Handle countdown
		pass
...
func _enter_state(state: GameState, data: Dictionary) -> void:
	...
	match state:
		...
		GameState.ROOM_SELECT:
			_load_scene("res://scenes/ui/RoomSelect.tscn")
...
func _load_scene(scene_path: String, _data: Dictionary = {}) -> void:
	EventBus.scene_transition_requested.emit(scene_path, _data)
...
func quit_to_host(reason: String = "player_exit") -> void:
  ...
  # Other messages handled by other systems