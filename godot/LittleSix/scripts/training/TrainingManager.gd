...
func confirm_training_day() -> void:
    ...
    for activity in selected_activities:
        var changes = racer.apply_training(activity)
        summary[str(activity)] = changes
        EventBus.training_activity_resolved.emit(activity, changes)
    
    # Roll for random event (25% chance)
    if randf() < 0.25:
        _trigger_random_event(racer)
...
func _trigger_random_event(racer: RacerData) -> void:
    # Simplified random event system
    var events = ["good_weather", "mechanical_issue", "inspiration", "minor_injury"]
    var event = events[randi() % events.size()]
    EventBus.training_random_event_fired.emit({"event": event})
    
    match event:
        "minor_injury":
            racer.is_injured = true
            racer.injury_days_remaining = 2
            EventBus.injury_occurred.emit("endurance", 2)
